        ...
        for iev,event in enumerate(events):
            ...
            if len(names) == 0: ## figure them out here
                ...
                for p in range(0,len(lhe.product().weights())):
                    if opts.minME in lhe.product().weights()[p].id.lower(): foundMin=True
                    if foundMin and not foundMax: names.append( (lhe.product().weights()[p].id, p) )
                    if opts.maxME in lhe.product().weights()[p].id.lower(): foundMax=True
                print ("names is now:", names)

                if len(names)==0: ## ERROR?
                    print ("ERROR: --------------- No names found ---------")
                    for p in range(0,len(lhe.product().weights())):
                        print ("DEBUG",lhe.product().weights()[p].id, "Looking for",opts.minME, "and",opts.maxME)
...
for i,(n,p) in enumerate(names):
    ...
    b=tree.Branch(n.lower(),values[i],n.lower()+"/D")
...
for i,(n,p) in enumerate(names):
    ...
    b=run_tree.Branch("sum_"+n.lower(),values_sums[i],"sum_"+n.lower()+"/D")
